...
timer_sleep (int64_t ticks)
{
  ...
  wake_up_time[num_of_alarm] = start + ticks;
  alarm_thread[num_of_alarm] = thread_current();
...
//  intr_set_level(old_level);
  ...
  while(i < num_of_alarm){
    if(timer_ticks()>= wake_up_time[i]){
      ...
      j = i;
      while(j < num_of_alarm - 1){
        wake_up_time[j] = wake_up_time[j+1];
        alarm_thread[j] = alarm_thread[j+1];
        j++;
      }
      num_of_alarm--;
    }
